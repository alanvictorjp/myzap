...
module.exports = class Sessions {
                ...
                if (session.client)
                    await session.client.then(async client => {
                        try {
                            ...
                            console.log("client.logout(): " + error.message);
                        }
                        session.state = "LOGOUT";
                        session.client = false;
                        console.log("client.logout - session.state: " + session.state);
                    });
                return { result: "success", message: "LOGOUT" };
            } else { //close
            ...
            if (session.state == "CONNECTED") {
                await session.client.then(async client => {
					if (!isNaN(params.number)) {
	                    console.log('#### delete chat =', params);
    	                return await client.archiveChat(params.number + '@c.us');
					}
                });
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                await session.client.then(async client => {
					if (!isNaN(params.number)) {
	                    console.log('#### delete chat =', params);
    	                return await client.deleteChat(params.number + '@c.us');
					}
                });
                return { result: "success" }
            } else {
    ...
    static async sendListMessage(req) {
        var params = {
            sessionName: req.body.sessionName,
            number: req.body.number
        }
            ...
            if (session.state == "CONNECTED") {
                await session.client.then(async client => {
					if (!isNaN(params.number)) {
						...
						return await client.sendListMessage(params.number + '@c.us', {
    ...
    static async clearChat(sessionName) {
        var session = Sessions.getSession(sessionName);
        if (session) {
            if (session.state == "CONNECTED") {
                var resultclearChat = await session.client.then(async (client) => {
                    return client.clearChat();
                });
                return {
                    result: resultclearChat
                };
            } else {
                return {
                    result: "error",
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var resultgetHostDevice = await session.client.then(async (client) => {
                    return client.getHostDevice();
                });
                return {
                    result: resultgetHostDevice
                };
            } else {
                return {
                    result: "error",
    ...
    static async getAllMessagesInChat(sessionName, number) {
        var session = Sessions.getSession(sessionName);
        //console.log(sessionName+number);
        if (session) {
            if (session.state == "CONNECTED") {
                var resultgetAllMessagesInChat = await session.client.then(async (client) => {
                    return await client.getAllMessagesInChat(number + '@c.us');
                });
                return {
    ...
    static async getMessages(sessionName, number) {
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var resultgetMessages = await session.client.then(async (client) => {
                    return await client.getMessages(number + '@c.us');
                });
                return {
